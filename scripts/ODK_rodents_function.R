clean_odk_rodent <- function(type = "rodent") {
  
  all_files <- list.files(here("data", "raw_odk", "rodent_data"), full.names = T)
  
  not_needed_vars <- c("SubmissionDate", "form_entry", "village_name", "rodent_numbering", 
  ...
  mutate(trap_number = case_when(rodent_number == 1 & village == "bambawo" & visit == 1 ~ 9,
         ...
         rodent_id = paste0(visit, "-", village_abbreviation, "-", study_site, str_pad(rodent_number, width = 3, pad = "0", "left"))) %>%
