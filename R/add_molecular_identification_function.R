add_molecular_identification <- function(data) {
  ...
  rodent_sequences <- combined_data$rodent_sequences %>%
    select(rodent_uid, species = blastn, qc = Result, sequence_identity = blastn_identity)
  
  rodent_sequences <- data %>%
    rename(field_id = clean_names) %>%
    left_join(rodent_sequences, by = c("rodent_uid")) %>%
    select(any_of(c("rodent_uid", "trap_uid", "research_visit", "species", "qc", "sequence_identity", "field_id", "genus", "sub_family", "family",
