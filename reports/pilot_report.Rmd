...
A recent review conducted by [@gibb_understanding_2017] collated published and unpublished documented Lassa Fever cases (humans) and infections (PCR) or seroprevalence (Ab) (rodents) from countries reporting LASV presence. This data has been used to support study site selection following production of Figure \@ref(fig:site_selection) and \@ref(fig:lassa_panguma).
...
```{r lassa_landuse, quietly=T, echo=F, fig.cap="Land use types in Sierra Leone, Eastern Sierra Leone (Middle) and land use in the buffer area of Lassa fever cases in Eastern Sierra Leone (2km radius) (R)."}
...
``` {r village_landuse, quietly = T, echo = F, fig.cap = "Land use at 100m resolution in a 2km radius from the centre of each village study site."}
# lalehun <- read_rds(here("data", "satellite", "lalehun_landuse_tmap.rds"))
# seilama <- read_rds(here("data", "satellite", "seilama_landuse_tmap.rds"))
